			...
			<div class="navbar-content-container">
				<div class="searchbar-container">
					<div class="searchbar">
						...
						<el-popover popper-class="searchbar-popover" trigger="focus" :show-arrow="false" placement="bottom">
							<template #reference>
								<input class="searchbar-input" placeholder="输入关键字搜索..." />
							</template>
							<template #default>
								<h4>no history...</h4>
							</template>
						</el-popover>
						<span class="searchbar-shortcut">Ctrl K</span>
					</div>
				</div>
				<div class="content-container">
					...
					<div class="theme-toggler">
						<div class="switch" @click="changeScheme">
...
function changeScheme() {
	const element = document.getElementsByTagName('html')[0].classList;
...
@media screen and (min-width: 960px) {
	...
	.searchbar {
		--searchbar-text-color: #ced4d9;
		--searchbar-suffix-background-color: #f9f9f9;
		--searchbar-suffix-border-color: #dbdbdb;
	}
	.dark .searchbar {
		--searchbar-suffix-border-color: #454847;
		--searchbar-suffix-background-color: #2a2c2c;
	}
	.searchbar {
		height: 28px;
	...
	.searchbar > svg {
		...
		fill: var(--searchbar-text-color);
	}
	.searchbar-input {
		width: 280px;
		height: 100%;
		padding: 0;
		padding-inline-start: calc(16px + 30px);
		padding-inline-end: calc(10px - 1px);
		font-size: 14px;
	...
	.searchbar-shortcut {
		...
		border-radius: 4px;
		pointer-events: none;
		border: 1px solid var(--searchbar-suffix-border-color);
		background-color: var(--searchbar-suffix-background-color);
		transition: opacity var(--translation-duration);
	}
	.searchbar-input:focus ~ .searchbar-shortcut {
		opacity: 0;
	}
	//   navmenu
	.content-container {
